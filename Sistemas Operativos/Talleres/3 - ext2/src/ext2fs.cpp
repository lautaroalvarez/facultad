...
unsigned int Ext2FS::get_block_address(struct Ext2FSInode * inode, unsigned int block_number)
{
	...
	int cantidad_bloque = block_size / sizeof(unsigned int);
	if (block_number < 12 + cantidad_bloque) {
		unsigned char block_buf[block_size];
		read_block(inode->block[12], block_buf);
		return block_buf[block_number - 12];
	}
	if (block_number < 12 + (cantidad_bloque * cantidad_bloque)) {
		unsigned char block_buf[block_size];
